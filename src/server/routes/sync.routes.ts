...
syncRouter.post("/", (req, res, next) => {
  try {
    ...
    if (!operations || !Array.isArray(operations)) {
      return res
        .status(400)
        .json({ success: false, message: "Invalid sync payload format" });
    }

    console.log(
      `[Sync API] Received ${operations.length} operations for background sync.`,
    );
    ...
    for (const op of operations) {
      ...
      try {
        ...
        if (
          url.includes("/api/attendance/manual") &&
          ...
          console.log(
            `[Sync API] Generic sync route fallback for: ${normalizedMethod} ${url}`,
          );
    ...
    return res
      .status(500)
      .json({ success: false, message: "Internal server error during sync" });
  }
});
