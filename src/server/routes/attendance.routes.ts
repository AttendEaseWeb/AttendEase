...
attendanceRouter.get("/", async (req, res, next) => {
  try {
    ...
    if (sessionId) {
      return res.json(
        await AttendanceService.getAttendanceBySession(sessionId as string),
      );
    }
    if (studentId) {
      return res.json(
        await AttendanceService.getAttendanceByStudent(studentId as string),
      );
    }
...
attendanceRouter.post("/manual", async (req, res, next) => {
  try {
    ...
    if (!sessionId || !studentId || !status) {
      return res
        .status(400)
        .json({ error: "Session ID, Student ID, and Status are required" });
    }
