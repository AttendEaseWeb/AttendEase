...
export class EmailService {
  static async sendParentNotification(
    ...
    if (!serviceId || !templateId || !publicKey || !privateKey) {
      console.warn(
        "EmailJS credentials not set in environment variables. Email not sent.",
      );
      return false;
    }

    try {
      const dateString = new Date(record.checkInTime).toLocaleDateString(
        "en-US",
        {
          weekday: "long",
          year: "numeric",
      ...
      const timeString = new Date(record.checkInTime).toLocaleTimeString(
        "en-US",
        {
          hour: "2-digit",
          minute: "2-digit",
      ...
      console.log(
        `Successfully sent EmailJS absence notification to ${parentEmail} for ${studentName}`,
      );
      return true;
    } catch (error) {
      console.warn(
        "Primary EmailJS account failed (possibly quota exceeded):",
        error,
      );
      ...
      if (
        ...
        try {
          ...
          console.log(
            `Successfully sent backup EmailJS notification to ${parentEmail} for ${studentName}`,
          );
          return true;
        } catch (backupError) {
          console.error(
            "Failed to send backup EmailJS notification:",
            backupError,
          );
          return false;
        }
