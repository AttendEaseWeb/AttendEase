...
export class ClassService {
  ...
  static async createClass(
    data: Omit<
      ClassSection,
      "id" | "createdAt" | "category" | "totalStudents"
    > & { category?: "JUNIOR_HIGH" | "SENIOR_HIGH"; subject?: string },
  ...
  static async createSession(
    data: Omit<ClassSession, "id" | "attendedCount">,
  ): Promise<ClassSession> {
    const cls = await dbStore.getClassById(data.classId);
    const primarySubject =
      cls?.subjects?.[0] || cls?.subject || data.subject || "General Class";
    ...
    const session: ClassSession = {
      ...
      attendedCount: 0,
      totalExpectedCount: cls
        ? cls.enrolledStudentIds.length
        : data.totalExpectedCount || 25,
    };
