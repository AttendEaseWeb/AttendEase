...
export const SectionDetailModal: React.FC<SectionDetailModalProps> = ({
  ...
  useEffect(() => {
    if (isOpen && cls) {
      ...
      setSelectedSubject(
        initialSubjects.length > 0 ? initialSubjects[0] : "ALL",
      );
  ...
  const fetchData = async (classId: string, studentIds: string[]) => {
    ...
    try {
      ...
      if (sessionsRes.ok) {
        ...
        const sectionSessions = allSessions.filter(
          (s) => s.classId === classId,
        );
  ...
  const handleAddSubjectToSection = async (subjectToAdd?: string) => {
    ...
    if (subjects.some((s) => s.toLowerCase() === targetSubject.toLowerCase())) {
      showToast(
        `Subject "${targetSubject}" is already added to this section`,
        "info",
      );
      setNewSubjectName("");
  ...
  const handleCreateSubjectSession = async (subjectName: string) => {
    try {
      ...
      const payload = {
        ...
        allowGeofence: true,
        totalExpectedCount:
          enrolledStudents.length || cls.enrolledStudentIds?.length || 25,
      };
  ...
  const activeSessionForSubject = filteredSessions.find(
    (s) => s.status === "ACTIVE",
  );
  ...
  const handleExportSubjectCSV = () => {
    ...
    if (recordsToExport.length === 0) {
      showToast(
        `No attendance logs recorded for ${selectedSubject === "ALL" ? "this section" : selectedSubject} yet`,
        "info",
      );
      return;
    }
    ...
    a.click();
    showToast(
      `Exported CSV report for ${selectedSubject === "ALL" ? cls.sectionName : selectedSubject}!`,
      "success",
    );
  };
  ...
  const suggestedAddList = (
    cls.gradeLevel <= 10 ? COMMON_JHS_SUBJECTS : COMMON_SHS_SUBJECTS
  ).filter(
    (subj) => !subjects.some((s) => s.toLowerCase() === subj.toLowerCase()),
  );

  return (
    <Modal isOpen={isOpen} onClose={onClose} title="" maxWidth="5xl">
        ...
        <div className="space-y-3 bg-m3-sys-light-surface dark:bg-m3-sys-dark-surface p-4 rounded-3xl border border-m3-sys-light-outline-variant/30 dark:border-m3-sys-dark-outline-variant/30 shadow-xs">
            ...
            <div className="p-3 bg-m3-sys-light-primary-container/30 dark:bg-m3-sys-dark-primary-container/30 rounded-2xl border border-m3-sys-light-primary/30 space-y-2 animate-fade-in">
              ...
              {suggestedAddList.length > 0 && (
                <div className="flex flex-wrap items-center gap-1.5 pt-1">
                  <span className="text-label-small font-medium text-m3-sys-light-on-surface-variant">
                    Quick Add:
                  </span>
          ...
          <div className="flex items-center gap-2 overflow-x-auto pb-1 pt-1 scrollbar-none">
            ...
            {subjects.map((subj) => {
              ...
              const hasActive = subjectSessions.some(
                (s) => s.status === "ACTIVE",
              );
              ...
              return (
                ...
                >
                  <BookOpen
                    className={`w-3.5 h-3.5 ${isSelected ? "text-m3-sys-light-primary" : "text-m3-sys-light-on-surface-variant"}`}
                  />
                  <span>{subj}</span>
                  {hasActive && (
                    <span
          ...
          <div className="p-5 rounded-3xl bg-m3-sys-light-surface dark:bg-m3-sys-dark-surface border border-m3-sys-light-outline-variant/30 dark:border-m3-sys-dark-outline-variant/30 space-y-4 shadow-sm">
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
              ...
              {canManage && (
                <div className="flex flex-wrap items-center gap-2">
                  ...
                  >
                    Launch{" "}
                    {selectedSubject === "ALL" ? "Attendance" : selectedSubject}{" "}
                    Session
                  </Button>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 pt-2">
              <div className="p-3 rounded-2xl bg-m3-sys-light-surface-variant/30 dark:bg-m3-sys-dark-surface-variant/30 border border-m3-sys-light-outline-variant/20">
                <div className="text-label-small font-medium text-m3-sys-light-on-surface-variant">
                  Sessions Conducted
                </div>
              ...
              <div className="p-3 rounded-2xl bg-emerald-500/10 border border-emerald-500/20">
                <div className="text-label-small font-medium text-emerald-700 dark:text-emerald-400">
                  Present Rate
                </div>
              ...
              <div className="p-3 rounded-2xl bg-amber-500/10 border border-amber-500/20">
                <div className="text-label-small font-medium text-amber-700 dark:text-amber-400">
                  Late / Absent Logs
                </div>
              ...
              {filteredSessions.length === 0 ? (
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                  {filteredSessions.map((sess) => {
                    ...
                    const sessRecords = attendanceRecords.filter(
                      ...
                      >
                        ...
                        <div className="flex items-center justify-between text-body-small text-m3-sys-light-on-surface-variant">
                          ...
                          <span className="font-semibold text-m3-sys-light-on-surface">
                            Attended: {sessRecords.length} /{" "}
                            {sess.totalExpectedCount ||
