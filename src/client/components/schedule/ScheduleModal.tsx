...
export const ScheduleModal: React.FC<ScheduleModalProps> = ({
  ...
  const [activeTab, setActiveTab] = useState<number>(
    new Date().getDay() === 0
      ...
      : new Date().getDay() > 5
        ? 1
        : new Date().getDay(),
  ); // Default to current day, skip weekends mostly
    ...
    if (isOpen && user) {
      ...
      const fetchSections = async () => {
        ...
        try {
          ...
          const mySections = data.filter(
            (s: ClassSection) => s.instructorId === user.id,
          );
  ...
  const currentDayEntries = schedule
    .filter((e) => e.dayOfWeek === activeTab)
    ...
    >
      <div className="flex flex-col space-y-6">
        <p className="text-body-medium text-m3-sys-light-on-surface-variant dark:text-m3-sys-dark-on-surface-variant">
          Configure your weekly schedule here. Classes matching the current time
          will be automatically highlighted on your Dashboard and Classes tabs.
        </p>
